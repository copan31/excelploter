...
from openpyxl.chart import Reference, Series, LineChart, AreaChart, BarChart, BubbleChart
import os
...
class ExcelPloter:
    ...
    def plot_bubble(self, sheetname):
        # 必要な値と項目を残した時系列データを作成
        df = self._df[["name", "age", "point"]]
        df["x"] = df["age"] * 2
        ...
        with pd.ExcelWriter(self._file) as writer:
            ...
            # plot line chart      
            self._plot_bubble(writer.book[sheetname], title = "バブル", xlabel = "日付", ylabel = "回数")
